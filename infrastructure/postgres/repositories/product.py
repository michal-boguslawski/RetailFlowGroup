...
class AlphaProductRepository(BaseRepository[Product, AlphaProductORM]):
    ...
    def _upsert_stmt(self, records: list[Product]) -> Insert:
    ...
    @staticmethod
    def _orm_to_model_mapper(orm: AlphaProductORM) -> Product:
        return orm_to_model(orm)

    @with_session
    def find_random(self, session: Session) -> Optional[Product]:
