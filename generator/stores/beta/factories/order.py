...
from domain.models import OrderEvent, Order, OrderLineItem, BetaUser
...
class BetaOrderFactory(BaseFactory[OrderEvent]):
    def __init__(
        self,
    ...
    def make_one(self, user: BetaUser, items: list[OrderLineItem], event_ts: int) -> OrderEvent:
        order = Order(
            id=self.id_generator.make_id("order_id"),
            user=user,
            guest_email=self.fake.email(),
            items=items,
            currency=self.fake.random_element([Currency.EUR, Currency.PLN]),
            notes=None
        )
