...
from domain.models import ClickstreamEvent, BetaUser
...
class BetaClickstreamFactory(BaseFactory[ClickstreamEvent]):
    def __init__(
        self,
    ...
    def make_one(self) -> ClickstreamEvent:
        ...
        country_code = self.fake.random_element(["DE", "PL"])
        ip_address = self.ip_generator.random_ip(country_code, self.fake.random_element([4, 6]))

        user = self.db_service.get_random("users")
        if not isinstance(user, BetaUser):
            raise TypeError(f"user is not of type BetaUser, but {type(user)}")
